    ...
    UUPSUpgradeable
{
    ...
    function _transfer(address from_, address to_, uint256 tokenId_, bool safe)
        internal
        onlyKeeper
        onlyKeeperHeld
        onlyKeeperSolvent
    {
        if (safe) {
            require(
                IERC721ReceiverUpgradeable(to_).onERC721Received(msg.sender, from_, tokenId_, "")
                    == IERC721ReceiverUpgradeable(to_).onERC721Received.selector,
    ...
    function transferFrom(address from, address to, uint256 tokenId_) public override {
        _transfer(from, to, tokenId_, false);
    }

    function safeTransferFrom(address from, address to, uint256 tokenId_) public override {
        _transfer(from, to, tokenId_, true);
    }

    function safeTransferFrom(address from, address to, uint256 tokenId_, bytes memory) public override {
        _transfer(from, to, tokenId_, true);
    }
