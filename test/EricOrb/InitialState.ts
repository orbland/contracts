...
export default function () {
    ...
    it("Should allow partial and full withdrawals", async function () {
        ...
        const gasSpentPartialWithdrawal = partialWithdrawalReceipt.gasUsed.mul(
            partialWithdrawalReceipt.effectiveGasPrice
        )
        ...
        expect(balanceAfterFullWithdrawal).to.be.equal(
            balanceBeforeWithdrawals
                .add(totalExpectedWithdrawal)
                .sub(gasSpentPartialWithdrawal)
                .sub(gasSpentFullWithdrawal)
        )
    })
    it("Should not allow withdrawals for more than deposited", async function () {
        ...
        await expect(orbUser.withdraw(defaultValue.mul(2))).to.be.revertedWithCustomError(
            orbDeployer,
            "InsufficientFunds"
        )
    ...
    it("Should not have any triggers set", async function () {
        ...
        expect(firstTrigger).to.be.eq(ethers.utils.formatBytes32String(""))
    })
}
