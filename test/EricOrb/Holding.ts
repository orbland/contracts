...
export default function () {
    ...
    it("Should correctly report effective funds", async function () {
        ...
        expect(await orbUser.effectiveFundsOf(deployer.address)).to.be.eq(
            ethers.utils.parseEther("0.025").add(ownerFunds)
        )
        ...
        expect(await orbUser.effectiveFundsOf(deployer.address)).to.be.eq(
            ownerFunds.add(ethers.utils.parseEther("0.05"))
        )
