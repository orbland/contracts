...
export default function () {
    ...
    it("Should allow checking if orb is triggerable", async function () {
        ...
        await expect(orbUser.triggerWithCleartext("what is 42?")).to.not.be.reverted
    ...
    it("Should not allow orb triggering before the cooldown expires", async function () {
        ...
        await expect(orbUser.triggerWithHash(triggerData)).to.not.be.reverted
        await time.increase((await orbDeployer.COOLDOWN()).sub(60 * 60)) // 1 hour before cooldown expires
        await expect(orbUser.triggerWithCleartext("what is 42?")).to.be.revertedWithCustomError(
            orbDeployer,
            "CooldownIncomplete"
        )
    })
    it("Should not allow anyone but the holder to trigger the orb", async function () {
        await afterFinalize.restore()
        expect(await orbUser.cooldownRemaining()).to.be.eq(0)
        await expect(orbUser2.triggerWithHash(triggerData)).to.be.revertedWithCustomError(orbDeployer, "NotHolder")
    })
    it("Should allow the holder to trigger the orb", async function () {
        ...
        await expect(orbUser.triggerWithCleartext("a".repeat(281))).to.be.revertedWithCustomError(
            orbDeployer,
            "CleartextTooLong"
        )
        await expect(orbUser.triggerWithHash(triggerData))
        ...
        expect(firstTrigger).to.be.eq(triggerData)
