...
contract WithdrawalAddressAuthorizationTest is OrbPondTestBase {
    event WithdrawalAddressAuthorization(address indexed withdrawalAddress, bool indexed authorized);

    function test_revertWhen_NotOwner() public {
        vm.prank(user);
        vm.expectRevert("Ownable: caller is not the owner");
        orbPond.authorizeWithdrawalAddress(address(0xBABEFACE), true);
    }

    function test_zeroAddressAlwaysAllowed() public {
        assertEq(orbPond.beneficiaryWithdrawalAddressPermitted(address(0)), true);
        vm.prank(owner);
        orbPond.authorizeWithdrawalAddress(address(0), false);
        assertEq(orbPond.beneficiaryWithdrawalAddressPermitted(address(0)), true);
    }

    function test_authorizeWithdrawalAddress() public {
        assertEq(orbPond.beneficiaryWithdrawalAddressPermitted(address(0xBABEFACE)), false);

        vm.expectEmit(true, true, true, true);
        emit WithdrawalAddressAuthorization(address(0xBABEFACE), true);
        vm.prank(owner);
        orbPond.authorizeWithdrawalAddress(address(0xBABEFACE), true);
        assertEq(orbPond.beneficiaryWithdrawalAddressPermitted(address(0xBABEFACE)), true);

        vm.expectEmit(true, true, true, true);
        emit WithdrawalAddressAuthorization(address(0xBABEFACE), false);
        vm.prank(owner);
        orbPond.authorizeWithdrawalAddress(address(0xBABEFACE), false);
        assertEq(orbPond.beneficiaryWithdrawalAddressPermitted(address(0xBABEFACE)), false);
    }
}
